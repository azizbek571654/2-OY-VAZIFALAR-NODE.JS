...
export const createShipmentValidation = [
  ...
  body('order_id')
    .notEmpty().withMessage('Buyurtma ID kiritilishi shart')
    .isUUID().withMessage('Buyurtma ID UUID formatida bo\'lishi kerak'),
  
  body('status')
    .optional()
    .isIn(['pending', 'in transit', 'delivered', 'failed'])
    .withMessage('Noto\'g\'ri status. Mumkin bo\'lgan qiymatlar: pending, in transit, delivered, failed'),
  
  body('tracking_number')
    .notEmpty().withMessage('Kuzatuv raqami kiritilishi shart')
    .isString().withMessage('Kuzatuv raqami matn formatida bo\'lishi kerak'),
  
  body('estimated_delivery')
    .notEmpty().withMessage('Yetkazib berish taxminiy kuni kiritilishi shart')
    .isISO8601().withMessage('Notogri sana formati. ISO 8601 (YYYY-MM-DD) formatida bo\'lishi kerak')
];
...
export const updateShipmentValidation = [
  body('status')
    .optional()
    .isIn(['pending', 'in transit', 'delivered', 'failed'])
    .withMessage('Notogri status. Mumkin bolgan qiymatlar: pending, in transit, delivered, failed'),
  
  body('tracking_number')
    .optional()
    .isString().withMessage('Kuzatuv raqami matn formatida bolishi kerak'),
  
  body('estimated_delivery')
    .optional()
    .isISO8601().withMessage('Notogri sana formati. ISO 8601 (YYYY-MM-DD) formatida bolishi kerak')
];
...
export const createUserValidation = [
    body('email')
        ...
        .isEmail().withMessage('Yaroqli email manzilini kiriting')
        .normalizeEmail(),
    
    body('username')
        .notEmpty().withMessage('Foydalanuvchi nomi kiritilishi shart')
        .isLength({ min: 3, max: 50 }).withMessage('Foydalanuvchi nomi 3-50 belgidan iborat bolishi kerak'),
    
    body('password')
        .notEmpty().withMessage('Parol kiritilishi shart')
        .isLength({ min: 6 }).withMessage('Parol kamida 6 belgidan iborat bolishi kerak'),
    
    body('role')
        .notEmpty().withMessage('Rol kiritilishi shart')
        .isIn(['customer', 'admin', 'manager']).withMessage('Rol quyidagilardan biri bolishi kerak: customer, admin, manager'),
    
    body('status')
        .optional()
        .isIn(['active', 'inactive', 'banned', 'suspended']).withMessage('Status quyidagilardan biri bolishi kerak: active, inactive, banned, suspended')
];
...
export const updatePasswordValidation = [
    ...
    body('new_password')
        ...
        .isLength({ min: 6 }).withMessage('Yangi parol kamida 6 belgidan iborat bolishi kerak')
        .custom((value, { req }) => {
            if (value === req.body.current_password) {
                throw new Error('Yangi parol joriy paroldan farq qilishi kerak');
            }
            return true;
        })
];
...
export const createOrderItemValidation = [
  body('order_id')
    .notEmpty().withMessage('Buyurtma ID kiritilishi shart')
    .isUUID().withMessage('Buyurtma ID UUID formatida bolishi kerak'),
  
  body('product_id')
    .notEmpty().withMessage('Mahsulot ID kiritilishi shart')
    .isUUID().withMessage('Mahsulot ID UUID formatida bolishi kerak'),
  
  body('quantity')
    .notEmpty().withMessage('Miqdor kiritilishi shart')
    .isInt({ min: 1 }).withMessage('Miqdor musbat butun son bolishi kerak'),
  
  body('price')
    .notEmpty().withMessage('Narx kiritilishi shart')
    .isFloat({ min: 0 }).withMessage('Narx musbat son b0lishi kerak')
];
...
export const updateOrderItemValidation = [
  ...
  body('price')
    .notEmpty().withMessage('Narx kiritilishi shart')
    .isFloat({ min: 0 }).withMessage('Narx musbat son bolishi kerak')
];