...
export const deleteCustomer = async (req, res) => {
  try {
    ...
    // Mijozning buyurtmalari mavjudligini tekshirish (agar orders jadvalingizda customer_id mavjud bo'lsa)
    const hasOrders = await query('SELECT COUNT(*) FROM orders WHERE customer_id = $1', [id]);
    
    // Agar mijozda buyurtmalar bo'lsa
    if (parseInt(hasOrders.rows[0].count) > 0) {
      return res.status(400).json({
        success: false,
        message: `Bu mijozda ${hasOrders.rows[0].count} ta buyurtma mavjud. Mijozni o'chirishdan oldin uning barcha buyurtmalarini o'chirish kerak`
      });
    }
